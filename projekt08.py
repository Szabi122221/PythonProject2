...
class KockaDobas:
    def __init__(self, master):
        ...
        # Dobások száma beviteli mező
        self.dobasok_szama_bemenet = tk.StringVar(value="10")
        self.dobasszam = tk.Entry(
            self.master, font=("Arial", 16), textvariable=self.dobasok_szama_bemenet, width=10
        )
        ...
        self.eredmeny_cimke = tk.Label(
            self.master, textvariable=self.eredmeny_cimke_szovege, font=("Arial", 16)
        )
        self.eredmeny_cimke.grid(row=2, column=1, pady=20)

        # Kilépés gomb
        self.kilepes = tk.Button(
            self.master, text="Kilépés", command=self.master.destroy, bg="red"
        )
    ...
    def on_dobas(self):
        try:
            ...
            messagebox.showerror("Hiba", "Rossz érték! Kérlek számot adj meg.")
